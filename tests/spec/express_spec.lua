...
describe("ExpressFramework", function()
  ...
  describe("Comment Filtering", function()
    ...
    it("should filter out single-line commented endpoints", function()
      local commented_content = '// app.get("/commented", handler)'
      local result = framework:parse(commented_content, "test.js", 1, 1)
    ...
    it("should filter out block commented endpoints", function()
      local commented_content = '* app.post("/block-commented", handler)'
      local result = framework:parse(commented_content, "test.js", 1, 1)
      assert.is_nil(result, "Block commented endpoint should be filtered out")
    end)

    it("should allow active endpoints", function()
      local active_content = 'app.get("/active", handler)'
      local result = framework:parse(active_content, "test.js", 1, 1)
    ...
    it("should filter various JavaScript comment styles", function()
      local test_cases = {
        '// app.get("/single-line", handler)',
        '    // app.post("/indented", handler)',
        '/* app.put("/block-start", handler) */',
        '* app.delete("/block-inside", handler)',
      }
